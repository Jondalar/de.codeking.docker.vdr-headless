...
sub ProcessEpg
{
    ...
    foreach $xmlline (@xmllines)
    {
        ...
        if ( ($xmlline =~ /\<programme/o ) && ( $xmlline !~ /clumpidx=\"1\/2\"/o ) && ( $chanevent == 0 ) )
        {
            ...
            $vdrid = ( $chanNb{$chanCur} * 100000 ) + ( $vdrst / 60 % 0xFFFF );
        ...
        # XML Image
        if ( ( $xmlline =~ /\<icon/o ) && ( $vdrid > 0 ) )
        {
            my ( $img ) = ( $xmlline =~ m/src\=\"(.*?)\"/ );
            $imgText .= "$vdrid $img\r\n";
        }
	...
	if ( ( $foundcredits== 1) && ( $xmlline =~ m/\<\/credits\>/o ) ) 
	{
        ...
        if ( ( $foundrating == 1 ) && ( $xmlline =~ m:\<value.*?\>(.*?)\<:o ) )
        {
            if ( $setrating == 0 )
            {
				...
				open(RATINGS, "/webgrab/ratings.conf") || die "cannot open file";
				my $ratingstxt;
				my $ratingsnum;
				my $ratingsline;
				my $tmp;
	...
	if ( $genre == 0 )
	{
		if ( ( $genreinfo == 0 ) && ( $xmlline =~ m:\<category.*?\>(.*?)\</category\>:o ) )
		{
			if ( $genre == $gi )
			{
				open(GENRE, "/webgrab/genres.conf") || die "cannot open file";
				my $genretxt;
				my $genrenum;
				my $genreline;
				my $tmp;
    ...
    if ( $atLeastOneEpg )
    {
        ...
        open(FH, '>', '/data/cache/images.data') or die $!;
        print FH $imgText;
        close(FH);
    }
}
